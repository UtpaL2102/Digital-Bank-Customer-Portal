...
router.post("/api/v1/scheduled-transfers/:id/cancel", async (req, res, next) => {
  try { const r = await accountClient.post(`/scheduled-transfers/${encodeURIComponent(req.params.id)}/cancel`, null, { headers: forwardContextHeaders(req) }); return res.status(r.status).json(r.data); }
  catch (e:any) { if (e.response) return res.status(e.response.status).json(e.response.data); next(e); }
});
